...
static void ShowWebViewDialog(int width, int height) {
    ...
    // height instead of the more compact fixed-dialog title bar, without
    // Minimize and Maximize so only Close shows; FixedFrameMessage keeps the
    // user from resizing it. WS_EX_APPWINDOW gives the unowned configuration
    // window its own taskbar button.
