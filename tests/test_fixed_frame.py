"""Run ChromeDevLauncher.c's fixed-size dialog frame against stubbed window calls.
...
PROC = 'WebViewWndProc'
CLASS = 'ChromeDevLauncherWebViewWnd'
...
class FixedFrameTests(unittest.TestCase):
    def test_frame_refuses_user_sizing(self):
        ...
        with tempfile.TemporaryDirectory(prefix='chromedevlauncher-frame-') as directory:
            path = Path(directory)
    ...
    def test_every_dialog_size_goes_through_the_frame(self):
        ...
        # The process is DPI-unaware, so Windows scales the window itself; a
        # WM_DPICHANGED resize, if one is ever added, must pin its size too.
        if 'WM_DPICHANGED' in proc:
